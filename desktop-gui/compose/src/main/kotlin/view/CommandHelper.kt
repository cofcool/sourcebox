...
@Composable
fun commandHelper() {
    val itemList = remember { mutableListOf<CommandItem>() }
...
fun searchCommand(items: List<CommandItem>, id: String?, tag: String?) {
...
fun commandItem(cmd: CommandItem) {
    Row(
        ...
        cmd.tags?.map { it }?.joinToString { "," }?.let { Text(it, modifier = Modifier.weight(2f)) }
    }
}