...
public class FileNameFormatter implements Tool {
    ...
    public void run(Args args) throws Exception {
        ...
        var listOpt = args.readArg("files").optVal();
        if (listOpt.isPresent()) {
            ...
            try {
                ...
                throw new IllegalStateException("Read files " + listPath + " error", e);
            }
        ...
        if (FileUtils.isDirectory(root)) {
            Files.walkFileTree(
                    ...
                    Integer.MAX_VALUE,
                    new DirEnterVisitor(nameGenerator, (n, f) -> rename(f, args, n, dryRun))
            );
        } else {
            rename(root, args, nameGenerator, dryRun);
        }
    }
    ...
    private void rename(File file, Args args, NameGenerator nameGenerator, boolean dryRun) {
        ...
        if (nameGenerator instanceof MoveGenerator && dest.isEmpty()) {
            throw new IllegalArgumentException("dest must not be empty when move");
        }
    ...
    public Args config() {
        return new Args()
                ...
                .arg(new Arg("files", null, "file contains list of absolute file paths, one per line", false, null))
